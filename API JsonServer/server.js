...
server.post('/auth/orders', (req, res) => {
  const {shoeId, customerName} = req.body
  const exist_shoe_id = db.shoes.findIndex(shoe => shoe.id === shoeId)
  ...
  const order_shoe = db.shoes[exist_shoe_id]
  if(order_shoe.available) {
    const new_order = {
